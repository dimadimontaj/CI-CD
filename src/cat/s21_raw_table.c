...
void cat_set_table(const char *table[static 256]) {
  const char *raw_ascii_hex_table[] = {
  ...
  memcpy(table, raw_ascii_hex_table, sizeof raw_ascii_hex_table);
}

void cat_set_endl(const char *table[static 256]) { table['\n'] = "$\n"; }

void cat_set_tab(const char *table[static 256]) { table['\t'] = "^I"; }
